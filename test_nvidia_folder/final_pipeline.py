...
#test_img_path = r"/home/jetson/Downloads/test_nvidia_folder/136922431.jpg"
test_img_path = r"/home/jetson/Downloads/test_nvidia_folder/127464350.jpg"
...
def pre_processing(image_path):
    ...
    # sharp_pil_image = Image.fromarray(cv2.cvtColor(sharp_image, cv2.COLOR_BGR2RGB))

    # # Save the compressed image
    # buffer = BytesIO()
    # sharp_pil_image.save(buffer, "JPEG", quality=60)
    # compressed_image_data = buffer.getvalue()
    # compressed_image_data = cv2.imdecode(np.frombuffer(compressed_image_data, np.uint8), cv2.IMREAD_COLOR)
    # print(type(compressed_image_data))
    # # Save the compressed image to a file named "jpeg_compressed.jpg"
    # compressed_file_path = 'jpeg_compressed.jpg'
    # with open(compressed_file_path, 'wb') as compressed_file:
    #     compressed_file.write(compressed_image_data)
        

    # return compressed_image_data, compressed_file_path
    return sharp_image
...
def leaf_isolation(image):
    ...
    # cv2.imshow("combined_leaves", combined_leaves)
...
def segmentation(leaves_image, output_dir):
    ...
    for i, contour in enumerate(contours):
        ...
        new_size = (int(0.5 * roi.shape[1]), int(0.5 * roi.shape[0]))  # 50% size
        roi = cv2.resize(roi, new_size)

        # Save the ROI to the output folder
        output_path = os.path.join(output_dir, 'leaf_{'+i+'.jpg')
        cv2.imwrite(output_path, roi)

    print(''+len(contours) +'leaves saved in' +output_dir)
    

cnn = load_model('./model_plant_detection.h5')
...
output_dir = r"/home/jetson/Downloads/test_nvidia_folder/output_folder"
...
print(""+not_healthy+"% plant is not healthy")

shutil.rmtree(r'/home/jetson/Downloads/test_nvidia_folder/output_folder')
